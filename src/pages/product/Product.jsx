...
import { useFindAllProduct } from "../../hooks/product/useFindAllProduct"
...
function Product() {
  ...
  const {docs: products, totalPages} = useFindAllProduct(page, limit, search)
  const {isLoading: isDeleleting, deleteProduct} = useDeleteProduct()

  const handleDelete = async () => {
      await deleteProduct(deleteId)
      const findIndex = products.findIndex((item) => item._id == deleteId)
                ...
                return (
                  <tr key={item._id} className="border-b border-gray-200">
                    ...
                    <td className="flex text-nowrap space-x-2 items-center">
                      <Link to={`/product/edit/${item._id}`} className="text-sm flex text-success items-center">
                        <FaRegEdit />
                        <span>កែប្រែ</span>
                      </Link>
                      <button onClick={() => {
